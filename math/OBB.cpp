...
OBB OBB::fromPointsMinVolume(const glm::vec3 *points,size_t count,float coarseStepDeg,float fineStepDeg,float ultraStepDeg)
{
    ...
    OBB best; if(points==nullptr||count==0) return best; if(count==1) { best.center=points[0]; return best; }

    // SoA 预处理，便于 AVX2 连续加载（glm::vec3 在当前编译设置下为 16 字节对齐，存在填充，不利直接批量 load）
