...
namespace exporters
{
    ...
    {
        std::vector<glm::mat4> world(model.mesh_nodes.size(), glm::mat4(1.0f));

        std::function<void(int32_t, const glm::mat4&)> rec = [&](int32_t idx, const glm::mat4 &parent)
        {
            if (idx < 0 || idx >= static_cast<int32_t>(model.mesh_nodes.size()))
                return;
            const auto &node = model.mesh_nodes[idx];
            glm::mat4 W = parent * node.node_transform.rawMat4();
            world[idx] = W;
