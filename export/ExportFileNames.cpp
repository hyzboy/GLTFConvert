...
namespace exporters
{
    // ------------------------------------------------------------
    // SanitizeName
    //  Keeps [A-Za-z0-9._-], replaces others with '_', collapses
    //  duplicated '_', trims leading/trailing '.' / '_', ensures
    //  non-empty, and avoids Windows reserved device names.
    // ------------------------------------------------------------
    std::string SanitizeName(std::string n)
    {
        ...
        for (char cOrig : n)
        {
            ...
            if (c == '_' && last == '_') continue; // collapse successive '_'
            out.push_back(c);
            last = c;
        }
