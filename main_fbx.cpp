...
{
    std::cout << "Usage: fbx_exporter [--no-images | --images-only] [--allow-u8-indices] <input.fbx> [output_dir]\n";
}
...
int main(int argc,char *argv[])
{
    ...
    FBXModel model;
    // Set option for allowing U8 indices before import/processing
    fbx::SetAllowU8Indices(allowU8);

    if(!fbx::ImportFBX(inputPath,model))
    {
        return 1;
    }
