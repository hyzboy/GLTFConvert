...
// Compare two primitives' geometry by accessor identity
static bool SameGeometryByAccessorId(const gltf::Geometry& a, const gltf::Geometry& b);

std::size_t PureModel::internBounds(const BoundingBox& b) {
...
std::size_t PureModel::internTRS(const MeshNodeTransform& t) {
...
std::size_t PureModel::internMatrix(const MatrixEntry& m) {
...
PureModel ConvertFromGLTF(const gltf::Model& src) {
    ...
    for (std::size_t ni = 0; ni < src.nodes.size(); ++ni) {
        ...
        if (node.mesh) {
            ...
            for (std::size_t primIndex : mesh.primitives) {
